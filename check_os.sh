...
OS_USED_ETH=`/usr/sbin/ip a | grep -w inet | egrep -v "lo$|docker_gwbridge|docker0|br-|virbr" | awk '{print $NF}'`
ETH_STAT_CODE=0
...
INTERFACE_LIST=`/usr/sbin/ip a | grep -w mtu | cut -d ":" -f 2 | egrep -v 'lo|inet|virbr|sit'`
